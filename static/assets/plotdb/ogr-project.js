  ...
  render: function() {
    ...
    this.allGroup.selectAll("path").each(function(d,i) {
      ...
      node.attr({
        ...
        d: function(d,i) { 
          return that.line(d,i) + (isfill ? "L0 0Z" : "");
        },
      });    
    }).transition("opacity").duration(500).attr({
      opacity: function(d,i) { return (!that.activeGroup || that.activeGroup == i+1 ? 1 : 0.1); }
